...
- 메뉴 (Menu)
    - [ ] 새로운 메뉴를 생성한다
        - 조건:
            - [ ] 메뉴의 가격은 0원 이상이어야 한다
            - [ ] 메뉴가 속한 메뉴 그룹이 존재하여야 한다
            - [ ] 매뉴에 포함한 메뉴 상품은 1개 이상이어야 한다
            - [ ] 메뉴에 포함한 메뉴 상품의 상품은 모두 존재하여야 한다
            - [ ] 포함한 모든 메뉴 상품 (MenuProduct)을 생성한다
            - [ ] 메뉴 가격이 전체 메뉴 상품의 가격 합보다 비싸면 안된다
            - [ ] 메뉴의 이름은 비여 있을 수 없다
            - [ ] 메뉴의 이름은 purgomalum API를 사용한 욕설, 비속어 등 부적절한 단어 포함 검사를 통과해야 한다
        - 결과:
            - [ ] 요청한 메뉴 이름으로 메뉴를 생성하여야 한다
            - [ ] 생성한 메뉴의 가격은 요청한 가격과 일치하여야 한다
            - [ ] 생성한 메뉴는 메뉴 그룹을 포함하고 있어야 한다
            - [ ] 생성한 메뉴의 기본으로 숨겨진 상태여야 한다
            - [ ] 생성한 메뉴는 요청한 메뉴 상품을 포함하고 있어야 한다
    - [ ] 특정 메뉴의 가격을 변경한다
        - 조건:
            - [ ] 변경할 가격은 0원 이상이어야 한다
            - [ ] 변경 대상인 메뉴이 존재하여야 한다
            - [ ] 변경할 가격이 변경 대상인 메뉴에 포함한 메뉴 상품의 가격 합보다 비싸면 안된다
    - [ ] 특정 메뉴를 표시한다
        - 조건:
            - [ ] 메뉴가 존재하여야 한다
            - [ ] 메뉴의 가격이 메뉴에 포함한 메뉴 상품의 가격 합보다 비싸면 안된다
    - [ ] 특정 메뉴를 숨긴다
        - 조건:
            - [ ] 매뉴가 존재하여야 한다
    - [ ] 모든 메뉴를 조회한다
- 주문 상세 항목 (OrderLineItem)
    - [ ] 주문 상세 항목을 생성한다
        - 조건:
            - [ ] 주문 상세 항목의 수량은 0개 이상이어야 한다
            - [ ] 주문 상세 항목의 메뉴는 존재하여야 한다
            - [ ] 주문 상세 항목의 메뉴는 표시된 상태여야 한다
    ...
    - 배달(`DELIVERY`)인 경우:
        ...
        - [ ] 주문을 접수 상태로 변경한다
            - 조건:
                ...
                - [ ] 배달 업체에 배달을 요청한다(외부 kitchenridersClient API 호출)
            - 결과:
    ...
    - 포장(`TAKE_OUT`)인 경우:
        - [ ] 주문을 생성한다
            - 조건:
                ...
                - [ ] 주문 상세 항목 (`OrderLineItem`)의 개수가 1개 이상이어야 한다
                - [ ] 주문 상세 항목 (`OrderLineItem`)마다 메뉴 아이디로 메뉴를 포함하고 있어야 한다
                - [ ] 주문 상세 항목 (`OrderLineItem`)의 개수와 메뉴의 개수가 일치해야 한다
                - [ ] 주문에 포함한 모든 주문 상세 항목 (`OrderLineItem`)을 생성한다
            - 결과:
        ...
        - [ ] 주문을 완료 상태로 변경한다
            ...
            - 결과:
                ...
                - 주문 테이블의 모든 주문이 완료 상태인 경우
                    ...
                    - [ ] 주문 테이블의 고객 수가 0명으로 변경되어야 한다
- 주문 테이블 (OrderTable)
    - [ ] 주문 테이블을 생성한다
        - 조건:
            - [ ] 주문 테이블의 이름은 비여 있을 수 없다
        - 결과:
            - [ ] 요청한 주문 테이블 이름으로 주문 테이블을 생성하여야 한다
            - [ ] 생성한 주문 테이블은 고객 수가 0명이어야 한다
    ...
    - [ ] 주문 테이블을 비운다
        - 조건:
            - [ ] 주문 테이블이 존재하여야 한다
            - [ ] 주문 테이블에 포함한 주문이 모두 완료 상태여야 한다
        - 결과:
            - [ ] 주문 테이블은 빈 상태로 변경되어야 한다
            - [ ] 주문 테이블의 고객 수가 0명으로 변경되어야 한다
    - [ ] 주문 테이블의 고객 수를 변경한다
        - 조건:
            - [ ] 변경할 고객 수는 0명 이상이어야 한다
            - [ ] 주문 테이블이 존재하여야 한다
            - [ ] 주문 테이블이 점유된 상태에만 고객 수를 변경할 수 있다
        - 결과:
            - [ ] 주문 테이블의 고객 수는 변경된 값으로 설정되어야 한다
    - [ ] 모든 주문 테이블을 조회한다
- 상품 (Product)
    - [ ] 상품을 생성한다
        - 조건:
            - [ ] 상품의 가격은 0원 이상이어야 한다
            - [ ] 상품의 이름은 비여 있을 수 없다
            - [ ] 상품의 이름은 purgomalum API를 사용한 욕설, 비속어 등 부적절한 단어 포함 검사를 통과해야 한다
        - 결과:
    ...
    - [ ] 상품의 가격을 변경한다
        ...
        - 결과:
            ...
            - 상품을 포함한 메뉴의 가격이 해당 메뉴에 포함한 메뉴 상품의 가격 합보다 비싸면
                - [ ] 메뉴의 상태를 숨김 상태로 변경한다
    - [ ] 모든 상품을 조회한다

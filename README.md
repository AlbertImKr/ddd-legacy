...
- 메뉴 (Menu)
    - [ ] 새로운 메뉴를 생성한다
        ...
        - 결과:
            ...
            - [ ] 생성한 메뉴의 표시 상태는 요청한 상태와 일치하여야 한다
            - [ ] 생성한 메뉴는 요청한 메뉴 상품을 포함하고 있어야 한다
    - [ ] 특정 메뉴의 가격을 변경한다
        - 조건:
            - [ ] 변경할 가격은 0원 이상이어야 한다
            - [ ] 변경 대상인 메뉴이 존재하여야 한다
            - [ ] 변경할 가격이 변경 대상인 메뉴에 포함된 메뉴 상품의 가격 합보다 비싸면 안된다
    - [ ] 특정 메뉴를 표시한다
        - 조건:
            - [ ] 메뉴가 존재하여야 한다
    ...
    - [ ] 특정 메뉴를 숨긴다
        - 조건:
            - [ ] 매뉴가 존재하여야 한다
        - 비고:
            - 조건이 필요 없는 이유는 메뉴 숨김 기능이 단순 표시 여부를 변경하는 것이기 때문이다
    - [ ] 모든 메뉴를 조회한다
- 주문 테이블 (OrderTable)
    - [ ] 주문 테이블을 생성한다
        - 조건:
    ...
    - 배달(`DELIVERY`)인 경우:
        - [ ] 주문을 생성한다
            - 조건:
                ...
                - [ ] 주문에 포함된 모든 주문 상세 항목의 개수는 0개 이상이어야 한다
                - [ ] 배달 주소는 비어 있을 수 없다
            - 결과:
